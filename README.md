...
**1.2.2** — Ruční změna **data odevzdání** v tabulce teď automaticky přepočítá sloupec **Pokus** (řádný / opravný / po termínu / neodevzdal) a tím pádem i **známku** — dosud zůstával pokus zaseknutý na původní hodnotě. Model `StudentTableModel` má nový `set_deadlines()` setter; MainWindow ho volá při loadu ročníku i při inline změně termínu ve statistice. `derive_pokus_from_date` přesunuto z `io/csv_import.py` do `domain/grading.py` (čistá doménová logika; v `io/csv_import.py` zůstal re-export pro testy).
**1.0.1** — Editor bodových sloupců (Test 1, Test 2, Projekt, Bonus) teď podporuje **3 desetinná místa** — nová `PointsDelegate` (QDoubleSpinBox s `decimals=POINTS_DECIMALS`, krok `0.001`, range podle max sloupce). Default Qt editor používal jen 2 desetiny a třetí ořezával, přestože storage byla na `POINTS_DECIMALS=3` od začátku. Spinbox navíc clampuje hodnotu na `MAX_TEST1` / `MAX_TEST2` / `MAX_PROJEKT` / `MAX_CELKEM`.
**1.0.0** — 🎉 **Milestone release.** Sloupec **CTFL** auto-resizuje podle obsahu (text „Ano" je v bold variantě širší než default 60 px to zvládl, ořezávalo se to). Po každém toggle checkboxu se přepočítá šířka přes `resizeColumnToContents` (signál `studentChanged` / `modelReset`), s bezpečným floorem na default šířce z `COLUMNS`. Model navíc returnuje `SizeHintRole` zahrnující šířku indicatoru + bold textu + `PM_CheckBoxLabelSpacing` + generózní padding, aby Qt mělo přesné info pro výpočet.
**0.10.5** — Checkbox v sloupci **Docházka** vycentrován (default Qt ho dává vlevo). Pozadí (`DOCHAZKA_OK_BG` / `DOCHAZKA_FAIL_BG` / `DOCHAZKA_REPETENT_OK_BG`), styl checkboxu i ostatní vizuál zůstávají nedotčené — používá se nativní `PE_IndicatorItemViewItemCheck` jen s posunutým rectem. Klik kdekoli v buňce toggle-uje hodnotu (UX, kterou uživatel u centrovaného checkboxu očekává). Nový delegate `CenteredCheckboxDelegate` v `ui/delegates.py` aplikovaný jen na sloupec Docházka.
**0.10.4** — Termíny odevzdání se editují **inline** ve Statistice ročníku (perzistentní `QDateEdit` widgety s kalendářovým pop-upem — přežijí rebuild statistik). Toolbar tlačítko **„Termíny…"** bylo odstraněno (dialog zůstává jen pro nový ročník). Validace stejná jako v dialogu: opravný termín musí být později než řádný (jinak warning + revert). Při změně deadlinů se automaticky přepočítá `pokus` u všech studentů, kteří mají datum odevzdání — řádný / opravný / po termínu se naladí podle nových dat, statistika i tabulka se okamžitě obnoví. `_derive_pokus_from_date` přejmenováno na public `derive_pokus_from_date` (sdílí ho teď CSV import a UI handler).
**0.10.3** — Předvolba nosného CSV při exportu: pokud už pro daný ročník existuje aspoň jeden archivovaný export, dialog **„📤 Export hodnocení (CSV)"** ho předvybere jako nosné CSV. Verze tak na sebe přirozeně navazují — každý další export staví na předchozím (`zk_*` z STAGu se postupně zaplňují/aktualizují, ostatních 15 sloupců zůstává netknutých). Uživatel může samozřejmě vybrat libovolné jiné CSV.
**0.10.2** — Indikátor stavu exportu se zobrazuje pouze u **aktuálního** akademického roku (`date.today().year`) — u starších ročníků je irelevantní, do STAGu se v nich už stejně nezapisuje. Indikátor navíc bere v potaz mazání archivovaných exportů: pokud uživatel ve **„📁 Exportované CSV…"** smaže všechny archivované exporty daného ročníku, indikátor se vrátí na žluté **„⚠ Zatím nebylo exportováno do STAGu"** (persistovaný hash odkazoval na soubor, který už neexistuje). Status bar se obnoví hned po zavření správce exportů.
**0.10.1** — Indikátor stavu exportu vůči STAGu v status baru. Po každém STAG exportu se do JSON ročníku uloží SHA-256 hash exportovatelných polí (`os_cislo`, `datum`, `pokus`, `známka`, `celkem`) a timestamp. Status bar pak ukazuje barevný stav: žluté **„⚠ Zatím nebylo exportováno do STAGu"** (ročník po importu), zelené **„✓ Synchronizováno se STAGem (před X)"** (vše uloženo), nebo červené **„● Neuložené změny od posledního exportu (před X)"** (něco se po exportu změnilo). Indikátor reaguje okamžitě na editace (signál `studentChanged`), čas je lidsky čitelný (`před chvílí` / `před 5 min` / `před 2 h` / `včera` / `před N dny` / datum). Nový modul `domain/export_state.py` (hash) a `domain/time_ago.py` (formátování).
